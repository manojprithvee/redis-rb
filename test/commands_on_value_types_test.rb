...
class TestCommandsOnValueTypes < Test::Unit::TestCase
  ...
  def test_migrate
    redis_mock(:migrate => lambda { |*args| args }) do |redis|
      options = { :host => "139.59.61.46", :port => 1234 }
      ...
      expected = ["139.59.61.46", "1234", "foo", default_db.to_s, default_timeout.to_s]
      assert_equal expected, actual

      # Test db override
      actual = redis.migrate("foo", options.merge(:db => default_db + 1))
      expected = ["139.59.61.46", "1234", "foo", (default_db + 1).to_s, default_timeout.to_s]
      assert_equal expected, actual

      # Test timeout override
      actual = redis.migrate("foo", options.merge(:timeout => default_timeout + 1))
      expected = ["139.59.61.46", "1234", "foo", default_db.to_s, (default_timeout + 1).to_s]
      assert_equal expected, actual
    end
  end
end
